...
| **4** | [market-data-and-analysis.cn.md](./market-data-and-analysis.cn.md) | 先广后窄：榜单 → Top N → `meta` + `dynamic` → 1～3 标的 K 线 → 可选流入/聪明钱；RWA 独立。 |
| **5** | [onchain-signals-and-security.cn.md](./onchain-signals-and-security.cn.md) | **有合约则先审计** → 信号 → 流动性检查 → 地址分页；Meme 叙事可对齐。 |
...
| **10** | [binance-square-posting.cn.md](./binance-square-posting.cn.md) | 撰稿定稿 → 可选行情事实 → 单次 `content/add`；定时 = 外部调度重复同流程。 |
