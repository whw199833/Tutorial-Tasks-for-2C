...
| **4** | [market-data-and-analysis.en.md](./market-data-and-analysis.en.md) | Broad first, then narrow: ranks → Top N → `meta` + `dynamic` → 1–3 symbols multi-interval K-lines → optional inflow / smart-money; RWA separate. |
| **5** | [onchain-signals-and-security.en.md](./onchain-signals-and-security.en.md) | **Audit first** if contract exists → signals → liquidity check → address pagination; Meme narrative alignment optional. |
...
| **10** | [binance-square-posting.en.md](./binance-square-posting.en.md) | Draft & compliance → optional market facts → single `content/add`; scheduling = external cron repeating the same flow. |
